...
- **Paramedir**: Tool to postproces Extrae's traces, developed by BSC's Performance Analysis group. It is part of the _Paraver_ package, which is available as source code or precompiled binaries for several operating systems.
- **Flexmalloc**: User-level interposition runtime for dynamic objects allocation on heterogeneous memory. 

Extrae and Paramedir can be download from the [AccelComm group's Github](https://github.com/accelcom-bsc/extrae) OR the [BSC's Performance Analysis group website](https://tools.bsc.es/downloads). It is recommended to get both Extrae and Paramedir from the first option, however, in case you choose the later, the minimum _Extrae_ version required is 4.0.0.

The Flexmalloc library can be download it from its [Github repository](https://github.com/intel/flexmalloc).
...
We recommend to use one of the Extrae XMLs provided under `extrae_confs/` and set the **ECOHMEM\_EXTRAE\_XML** environment variable in [Step 1](https://gitlab.bsc.es/-/ide/project/accelcom/software/heterogeneus-memory/placement/hmem-advisor/edit/master/-/readme.md#1-setup-configuration) to its path:
...
- **<merge ..>**: If enabled, it converts the intermediate Extrae trace to the usual prv format. If it is not enabled, it will be performed by the `profile_run.sh` script of [Step 1](https://gitlab.bsc.es/-/ide/project/accelcom/software/heterogeneus-memory/placement/hmem-advisor/edit/master/-/readme.md#1-setup-configuration). In the later case, make sure to set the necessary environment vairables of **ECOHMEM\_MPI2PRV** and **ECOHMEM\_MPI2PRV\_EXTRA\_FLAGS** in [Step 1](https://gitlab.bsc.es/-/ide/project/accelcom/software/heterogeneus-memory/placement/hmem-advisor/edit/master/-/readme.md#1-setup-configuration) of the workflow.
...
:mailbox: AccelCom Email: [accelcom@bsc.es](mailto:accelcom@bsc.es)
